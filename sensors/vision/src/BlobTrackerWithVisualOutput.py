...
class BlobTrackerWithVisualOutput :
    
    #def __init__( self ) :
        #self.capture = cv.LoadImageM("orangeCone2.jpg")
    ...
    def searchFrameForColorBlob(self, idealRed, idealGreen, idealBlue, searchColorThreshold):
        ...
        img = cv.LoadImageM("orangeCone2.jpg")
        ...
        blobParamArray = [mask, 0, 0.0, 0.0]  # mask, totalPixels, xAvg, yAvg. 
        ...
        # Display the window
        cv.ShowImage("MyWindow", img)
        # Wait until key is pressed to exit
        cv.waitKey(0);
...
#                return [trackingCoordinates, img]
#
...
def findSurroundingPixelGroups(img, xCord, yCord, colorTolerance, maskValue, blobParamArray, recuriveCount, groupSize):
    ...
    print 'xCord%d totalNumberOfGroupPixels=%d xAvg=%.5f totalPixels=%d' % (xCord,totalNumberOfGroupPixels,xAvg,totalPixels)
